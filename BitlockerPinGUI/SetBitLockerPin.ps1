...
# Add file to C:\Program Data\Bitlocker\SetBitLockerPin.log
$logPath = "C:\ProgramData\Bitlocker"
If (!(Test-Path -Path $logPath)) {
    New-Item -Path $logPath -ItemType Directory | Out-Null
}
$logFile = "$(Join-Path -Path $logPath -ChildPath "SetBitLockerPin.log")"
$timeStamp = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
If ($exitCode -eq 0 -And (Test-Path -Path $pathPINFile)) {
    Add-Content -Path $logFile -Value "$timeStamp - BitLocker PIN set successfully."
} Else {
    Add-Content -Path $logFile -Value "$timeStamp - BitLocker PIN not set."
}